...
class UserDbHandler(DbHandler):
    ...
    def get(db: DbQuery, idName: str, id: str) -> User:
        try:
            userData = db.get(UserDbHandler._table_name, 'userID', id)
            if len(userData) != 1:
                return None
    ...
    def delete(db: DbQuery, idName: str, id) -> bool:
        try:
            ...
            return db.delete(UserDbHandler._table_name, 'userID', id)
        except Exception:
            return False
    ...
    def add(db: DbQuery, obj: User) -> bool:
        try:
            dictData = obj.toJson()
            return db.add(UserDbHandler._table_name, dictData)
        except Exception:
            return False
    ...
    def update(db: DbQuery, obj: User) -> bool:
        try:
            dictData = obj.toJson()
            return db.update(UserDbHandler._table_name, dictData, 'userID', obj.getUserID())
        except Exception:
            return False
